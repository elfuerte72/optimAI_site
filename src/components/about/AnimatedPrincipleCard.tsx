...
const AnimatedPrincipleCard = forwardRef<HTMLDivElement, AnimatedPrincipleCardProps>(
  ({ title, text, icon: Icon, className }, ref) => {
    // 'ref' is the root div for this component
    ...
    return (
      ...
      <div
        ref={ref}
        className="principle-item flex w-full flex-wrap items-center py-4 md:flex-nowrap md:space-x-4"
      >
        {/* Card 1: Title and Icon */}
        <div
          className={cn(
            'relative rounded-xl border border-white/10 bg-neutral-900/60 p-5 shadow-lg backdrop-blur-md sm:p-6',
            'min-w-[220px] flex-shrink-0 sm:min-w-[250px]', // Ensure title card doesn't shrink too much
            className
          )}
        >
          <div className="flex items-center space-x-3">
            <Icon className="h-6 w-6 flex-shrink-0 text-sky-400 sm:h-7 sm:w-7" />
        ...
        <div
          ref={lineRef}
          className="h-1 rounded-full bg-gradient-to-r from-sky-400 to-indigo-600 opacity-0"
          style={{ width: '0px' }}
        />
        ...
        <div
          ref={textCardRef}
          className="flex-grow rounded-xl border border-white/5 bg-neutral-800/70 p-5 opacity-0 shadow-md backdrop-blur-md sm:p-6"
        >
          <p className="text-sm leading-relaxed text-neutral-300 sm:text-base">{text}</p>
        </div>
      </div>
    );
  }
