...
const PrinciplesSection: React.FC = () => {
  ...
  useGSAP(() => {
    ...
    principleItemRefs.current.forEach((itemRef, index) => {
      if (itemRef) { // Проверяем, что ссылка заполнена
        gsap.fromTo(
          itemRef,
          { opacity: 0, y: 40 },
          {
  ...
  return (
    <div ref={sectionRef} className="principles-section mt-24 sm:mt-32 mb-20 space-y-10 sm:space-y-16">
      <AnimatedSectionTitle ref={titleRef}>
        Наши принципы
      </AnimatedSectionTitle>
      
      {/* Стек элементов принципов вертикально. Каждый элемент - это ряд: [Карточка заголовка] --Линия-- [Карточка текста] */}
      <div className="flex flex-col items-center w-full px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
